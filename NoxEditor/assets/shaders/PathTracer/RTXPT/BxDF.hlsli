...
struct StandardBSDFData
{
    ...
    [mutating] void        SetEta(lpfloat eta)                                     { _transmission_eta.y = (_transmission_eta.y & 0xFFFF) | (f32tof16((float)eta)<<16); }
    // this is needed when limiting roughness in some denoising cases
    [mutating] void        SetRoughness(lpfloat roughness)                         { _diffuse_roughness.y = (_diffuse_roughness.y & 0xFFFF) | (f32tof16((float)roughness)<<16); }
#else
...
#endif
    // this is needed when updating interior<->exterior transitions
    [mutating] void        SetEta(lpfloat eta)                                     { _eta = eta; }
    // this is needed when limiting roughness in some denoising cases
    [mutating] void        SetRoughness(lpfloat roughness)                         { _roughness = roughness; }
#endif
};
...
struct FalcorBSDF // : IBxDF
{
    ...
    */
        ...
        const StandardBSDFData data)
    {
        ...
        pDiffuseReflection = (activeLobes & (uint)LobeType::DiffuseReflection) != 0 ? diffuseWeight * dielectricBSDF * (1.f - diffTrans) : 0.f;
        pDiffuseTransmission = (activeLobes & (uint)LobeType::DiffuseTransmission) != 0 ? diffuseWeight * dielectricBSDF * diffTrans : 0.f;
        pSpecularReflection = (activeLobes & ((uint)LobeType::SpecularReflection | (uint)LobeType::DeltaReflection)) != 0 ? specularWeight * (metallicBRDF + dielectricBSDF) : 0.f;
        pSpecularReflectionTransmission = (activeLobes & ((uint)LobeType::SpecularReflection | (uint)LobeType::DeltaReflection | (uint)LobeType::SpecularTransmission | (uint)LobeType::DeltaTransmission)) != 0 ? specularBSDF : 0.f;
    ...
    \param[in] data BSDF parameters.
*/
    [mutating] void initialize(const ShadingData shadingData, const StandardBSDFData data)
    {
        initialize(shadingData.mtl, shadingData.V, shadingData.N, data);
    }

    static FalcorBSDF make( const ShadingData shadingData, const StandardBSDFData data )     { FalcorBSDF ret; ret.initialize(shadingData, data); return ret; }
