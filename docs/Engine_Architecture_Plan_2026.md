...
| **Model instances** (§5.11.5, §5.13) | Phase 5b | Imported models are dragged in as one entity per glTF node (Bistro: 6007 entities in the scene file). Material lists already store overrides only (the per-entity copy of the mesh's 551 handles made Bistro's scene 54 MB and its load take minutes), so the scene stays usable meanwhile. | The engines' model: Unity's Model Prefab instance, Godot's instanced PackedScene, Unreal's Blueprint / Packed Level Actor from Interchange or Datasmith. A scene stores one root with a model reference and per-node overrides only (transform, material, removed or added nodes); loading spawns the hierarchy from the cooked node data the .nmesh already holds. Together with the binary cooked scene format of D12 for runtime. | 5c's async drag-in (entities created from cooked node data), or scenes with several large models. |
