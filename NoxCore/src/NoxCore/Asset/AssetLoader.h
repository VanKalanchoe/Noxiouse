...
namespace Nox
{
    ...
    // Background loads of cooked assets (§5.11): file reads on the IO workers, decoding on the CPU workers, GPU uploads
    // through the renderer's staging under a per-frame byte budget, and each resource published once its copies have
    // completed on the GPU. Owned and driven by the asset manager, main thread.
    class AssetLoader
    {
    public:
        AssetLoader();
        ...
        // Cooked textures and meshes, materials, skeletons and animations; everything else imports synchronously.
